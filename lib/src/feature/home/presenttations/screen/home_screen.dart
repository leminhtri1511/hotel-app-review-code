...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _handleTabChange(int index) {
    ...
    switch (index) {
      ...
      case 2:
        ...
        break;
      case 3:
        BlocProvider.of<HomeBloc>(context).add(const GetRoomsByStatus(3));
        break;
    }
  }
  ...
  Widget buildTabFour(HomeBloc bloc, HomeState state) {
    final sizeList = state.listHomeGetModel?.length ?? 0;
    return RefreshIndicator(
      onRefresh: () async {
        bloc.add(const GetRoomsByStatus(3));
      },
      child: Column(
        children: [
          Expanded(
  ...
  Widget buildGridView(
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final floorNumber = filteredRooms?[index].floorNumber;
        final roomName = filteredRooms?[index].roomName;
        final roomId = filteredRooms?[index].id;
        return Padding(
          ...
          child: GestureDetector(
            ...
            child: RoomItems(
                ...
                if (result == 'edit') {
                  ...
                  await showDialog(
                    ...
                    builder: (context) => WarningDialog(
                      ...
                      title: 'Xoá phòng',
                      content:
                          'Xác nhận xoá phòng "$floorNumber.$roomName" khỏi danh sách phòng',
                      leftButtonName: 'Trở về',
                      rightButtonName: 'Xác nhận',
                      onTapLeft: () => Navigator.pop(context),
                      onTapRight: () {
                        bloc.add(
                          HomeDeleteRoom(roomId),
                        );
                  ...
                  bloc.add(PutRoomStatusEvent(roomId, 3));
                  Timer(
                    const Duration(seconds: 1),
                    () => loadList(currentTabStatus),
                  );
                } else if (result == 'complete') {
                  bloc.add(PutRoomStatusEvent(roomId, 0));
                  print(roomId);
                  Timer(
                    const Duration(seconds: 1),
                    () => loadList(currentTabStatus),
              ...
              dropDownItemBuilder: (context) => currentTabStatus == 0
                  ? <PopupMenuEntry<String>>[
                      ...
                      const PopupMenuItem<String>(
                        ...
                        child: ListTile(
                          ...
                          title: Paragraph(content: 'Bảo trì'),
                        ),
                      ),
                    ]
                  : <PopupMenuEntry<String>>[
                      const PopupMenuItem<String>(
                        ...
                        child: ListTile(
                          ...
                          title: Paragraph(content: 'Hoàn tất'),
                        ),
                      ),
                    ],
              categoryRoomName: categoryName,
